...
import { BedDouble, Bath, Maximize, MapPin, Trees } from "lucide-react";
import { formatPrice, formatBaths } from "@/lib/listings";
import { formatSoldDate, type SoldListing } from "@/lib/sold";

/**
 * One sold-listing card. Shared by the homepage `RecentlySold` section and the
 * `/sold` page so the two can never drift. beds/baths/sqft and type render only
 * when the source published them (0 / omitted = Zillow gave no figure).
 */
export default function SoldCard({ listing }: { listing: SoldListing }) {
